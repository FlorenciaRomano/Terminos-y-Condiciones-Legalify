  ...
  let promiseResult: Promise<IPlanCicloPlanAmount> = new Promise((resolveResult, rejectResult) => {
    let promise: Promise<string> = new Promise((resolve, reject) => {
      ...
      if (paramPais && paramPais != "") {
        console.log(`Obtencion del pais por parametro en URL: ${paramPais}`);
        resolve(paramPais);
      } else {
        console.log(`Obtencion del pais por IP`);
        fetch(`${process.env.NEXT_PUBLIC_URL_APP_API}/multipago/util/json`)
          .then((resp) => resp.json())
          .then((resp) => {
            console.log(`consulta IP: ${resp.countryCode}`);
    ...
    promise.then((countryCode) => {
      console.log(`Pais seleccionado: ${countryCode}`);
      getPlanAmountsCountry2Digits(countryCode, ciclo).then((resp: IPlanAmount[]) => {
        ...
        console.log("Respuesta precios:", resp);
        resp.forEach(row => {
